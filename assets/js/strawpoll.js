            ...
            fn: function () {
                ...
                trumbowyg.openModal('Saisir les détails du StrawPoll', modalContent, function () {
                    ...
                    var settings = {
                        ...
                        headers: {
                            ...
                            'X-API-Key': '123' // Remplacez par votre clé API
                        },
                        processData: false,
                        data: JSON.stringify(pollData),
                    };

                    // Faire une requête POST vers l'API StrawPoll
                    $.ajax(settings)
                        .done(function (response) {
                            // Insérer le StrawPoll dans l'éditeur
                            var strawPollHtml = '<iframe src="' + response.url + '"></iframe>';
                            trumbowyg.execCmd('insertHTML', strawPollHtml);
                        })
                        .fail(function (error) {
                            console.error('Erreur lors de la création du StrawPoll : ' + error);
                        });
                ...
                $('#customConfirm').on('click', function () {
                    ...
                    var settings = {
                        ...
                        headers: {
                            'Content-Type': 'application/json',
                            'X-API-Key': 'e2357040-71cb-11ee-ba08-a87f99aa68de' // Remplacez par votre clé API
                        },
                        processData: false,
                        data: JSON.stringify(pollData),
                    };
