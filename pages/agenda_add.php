...
if (strpos($users['edu_group'], 'BUT1') !== false) {
    $sql_subject = "SELECT rs.*, ss.name_subject FROM sch_ressource rs
    ...
    $sql_subject = "SELECT r.*, s.name_subject 
    FROM sch_ressource AS r
    LEFT JOIN sch_subject AS s ON r.id_ressource = s.id_ressource ORDER BY name_subject ASC";
}
